...
# Sequence Turn Logic for CROSSROADS ONLY: -1 for left, 1 for right, 0 for straight
turn_sequence = [-1, 1, 0]
turn_index = 0
# ─────────────────────────────────────────────────────────────────────────────
                    ...
                    if junc_state == JUNC_FOLLOW:
                        ...
                        if now - junc_phase_start >= JUNC_STOP_DURATION:
                            ...
                            if current_label == "crossroad":
                                # [NEW] Crossroad: Execute sequence logic (Left -> Right -> Straight)
                                ...
                                # [NEW] T-junction: Use a fixed safe turn (Default to Left: -1)
                                # If you want it to always turn right at a T-junction, change -1 to 1 below.
                                junc_turn_sign = 1
                                junc_state = JUNC_TURNING
                                junc_phase_start = now
                                print(f"[JUNC] T-Junction. Action: Turning "
                                      f"{'LEFT' if junc_turn_sign == 1 else 'RIGHT'} (Fixed) "
                                      f"for {JUNC_TURN_DURATION:.2f} s")
