...
# ---------------- Config -------------------------
IMG_DIR     = "images"
TRAIN_CSV   = "train.csv" # training data
VAL_CSV     = "val.csv" # validation data
TEST_CSV    = "test.csv" # test data
IMG_SIZE    = 128
BATCH_SIZE  = 32
EPOCHS      = 50
LR          = 1e-3
...
class RoadDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(self.img_dir / row["image_name"]).convert("RGB")
        if self.transform:
            image = self.transform(image)
        return image, label2idx[row["label"]]

# Load dataset
def make_loader(csv_path, tf, shuffle):
    ds = RoadDataset(csv_path, IMG_DIR, tf)
    return DataLoader(
...
if __name__ == "__main__":
    ...
    train_loader = make_loader(TRAIN_CSV, train_tf, shuffle=True)
    val_loader   = make_loader(VAL_CSV,   eval_tf,  shuffle=False)
    test_loader  = make_loader(TEST_CSV,  eval_tf,  shuffle=False)
...
'''
# Predict (only used when testing)
