...
# =========================================================
# CNN model (single-channel version)
# =========================================================
...
def threshold_and_find_blob(gray_sm: np.ndarray, vision: QBPVision):
    """
    Return:
        error_px: float or None
        binary:   ROI binary image
        debug_roi: ROI visualization image
        blob_info: dict with col,row,area if found
    """
    h, w = gray_sm.shape
    ...
    # convert ROI-local blob center to error around image center
    img_center_x = w / 2.0
    error_px = img_center_x - float(col)

    # draw blob marker for debug
...
def main():
    ...
    print("CNN input mode: SINGLE-CHANNEL GRAYSCALE")
    ...
    try:
        ...
        print("Press 7 to toggle auto line following. Space to arm. U to quit.")
        ...
        while no_kill:
            ...
            if auto_mode:
                if line_error is None:
                    ...
                    speed_cmd = params["speed"]
                    commands = np.array([speed_cmd, steer_cmd], dtype=np.float64)
            ...
            if frame_count % CNN_INFER_EVERY == 1:
                if blob_info is None:
                    ...
                    blob_msg = f"blob_col={blob_info['col']:.1f}, area={blob_info['area']:.0f}"

                print(
                    f"[Scene] {stable_class:>10s} | conf={current_conf:.3f} | "
                    f"auto={auto_mode} | {blob_msg} | "
                    f"kp={params['kp']:.4f}, kd={params['kd']:.4f}, "
                    f"speed={params['speed']:.3f}, bias={params['bias']:.1f}"
                )
