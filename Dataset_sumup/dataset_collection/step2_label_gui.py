#-----------------------------------------------------------------------------#
#                         Step 2 - Interactive Trajectory Labeling GUI         #
#                                                                             #
#  Run this script after completing Step 1:                                   #
#    python data_collection/step2_label_gui.py                                #
#                                                                             #
#  How to operate:                                                            #
#    1. Automatically reads dataset/raw/coords.csv and plots the QBot path    #
#    2. Select label type on the right panel (crossroad / corner / straight)  #
#    3. Click and drag left mouse button on the trajectory plot to draw circles#
#    4. Repeat until all key regions are labeled                              #
#    5. Click "Save Regions" to save definitions to dataset/raw/regions.json  #
#    6. Step 3 reads this JSON file for batch labeling                        #
#-----------------------------------------------------------------------------#
...
import numpy as np
import matplotlib
matplotlib.use('TkAgg')          # Supports interaction; change to 'Qt5Agg' if error occurs
...
DEFAULT_RADIUS = 0.12   # Default circular region radius (meters)

def load_coords(csv_path):
    """Read coords.csv and return three lists: (image_names, xs, ys)"""
    names, xs, ys = [], [], []
...
class RegionLabeler:
    ...
    def _on_release(self, event):
        ...
        self._update_stat_text()
        self.fig.canvas.draw_idle()
        print(f"[Step2] New region added: {region}")
